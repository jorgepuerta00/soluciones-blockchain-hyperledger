...
echo -e "\e[1;35mℹ️  Generating CA containers for orderes and organizations\e[0m"
...
echo -e "\e[1;35mℹ️  Generating the genesis block\e[0m"
...
if [ $? -ne 0 ]; then
    echo -e "\e[1;31m❌ failed to generated genesis block\e[0m"
    return 1  
fi
...
echo -e "\e[1;35mℹ️  Starting hyperledger network orderes, peers and couchdb\e[0m"
...
echo -e "\e[1;35mℹ️  Joining orders & peers to the channel\e[0m"
sleep 10
...
peer version
if [ $? -ne 0 ]; then
    echo -e "\e[1;31m❌ failed to get peer version\e[0m"
    return 1  
fi

# Compile chaincode
echo -e "\e[1;31m🚨 IMPORTANT: Remember compile chaincode before packing.\e[0m"
...
peer lifecycle chaincode checkcommitreadiness \
    ...
    --output json
if [ $? -ne 0 ]; then
    echo -e "\e[1;31m❌ failed checkcommitreadiness chaincode\e[0m"
    return 1  
fi
...
if [ $? -ne 0 ]; then
    echo -e "\e[1;31m❌ failed commiting chaincode\e[0m"
    return 1  
fi
...
peer lifecycle chaincode querycommitted \
    ...
    --cafile ${PWD}/organizations/ordererOrganizations/universidades.com/orderers/orderer.universidades.com/msp/tlscacerts/tlsca.universidades.com-cert.pem
if [ $? -ne 0 ]; then
    echo -e "\e[1;31m❌ failed querycommitted chaincode\e[0m"
    return 1  
fi
...
if [ $? -ne 0 ]; then
    echo -e "\e[1;31m❌ failed Invoking InitLedger chaincode\e[0m"
    return 1  
fi
...
if [ $? -ne 0 ]; then
    echo -e "\e[1;31m❌ failed Invoking GetAllStudents chaincode\e[0m"
    return 1  
fi
...
if [ $? -ne 0 ]; then
    echo -e "\e[1;31m❌ failed Invoking CreateStudent chaincode\e[0m"
    return 1  
fi
